...
type googleController struct {
	socialUseCase auth.SocialAccountUseCase
	userUseCase   user.UserUseCase
}

func NewGoogleController(socialRepo auth.SocialAccountRepository, userRepo user.UserRepository) *googleController {
	return &googleController{
		socialUseCase: auth.NewSocialAccountUseCase(socialRepo),
...
func (c *googleController) PostGoogleCheck() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		sa, err := c.socialUseCase.FindOneBySocialIdAndProvider(
			ggp.SocialID,
...
func (c *googleController) PostGoogleSignIn() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		u, err := c.userUseCase.FindOneById(sa.UserID)
		if err != nil {
			rw.Error(err)
			return
		}
...
func (c *googleController) PostGoogleSignUp() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		sameNameUser, err := c.userUseCase.FindOneByUsername(params.Username)
		...
		userId, err := c.userUseCase.Create(&u)
		if err != nil {
			rw.Error(err)
			return
		}

		user, err := c.userUseCase.FindOneById(userId)
		if err != nil {
			rw.Error(err)
			return
		}
		...
		_, err = c.socialUseCase.Create(&sa)
		if err != nil {
			rw.Error(err)
			return
		}
